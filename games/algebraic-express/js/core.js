/* Algebraic Express — core: namespace, helpers, storage, icons, synthesized sound. */
(function () {
  'use strict';
  const AE = (window.AE = window.AE || { data: {}, meta: {}, game: {}, ui: {} });
  ...
  const SAVE_KEY = 'ae.save.v1';
  const SETTINGS_KEY = 'ae.settings.v1';
  const store = {
    ...
    loadSettings() { try { return Object.assign({ volume: 0.6, muted: false, shake: true }, JSON.parse(localStorage.getItem(SETTINGS_KEY) || '{}')); } catch (e) { return { volume: 0.6, muted: false, shake: true }; } },
    ...
    importCode(code) {
      ...
      if (!t.startsWith('AE1:')) throw new Error('That is not an Algebraic Express save code (it should start with AE1:).');
      const bin = atob(t.slice(4));
      const bytes = new Uint8Array(bin.length);
      for (let i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
      const s = JSON.parse(new TextDecoder().decode(bytes));
      if (!s || typeof s.v !== 'number' || !s.heroes) throw new Error('The save code is damaged or incomplete.');
      return s;
    },
  };
  ...
  const P = {
    ...
    cup: '<path d="M5 8h12v5a6 6 0 0 1-12 0z"/><path d="M17 9h1.5a2.5 2.5 0 0 1 0 5H17M8 4c0 1 1 1 1 2M12 4c0 1 1 1 1 2"/>',
    ...
    princess: '<path d="M7 9l2-4 3 3 3-3 2 4"/><circle cx="12" cy="13" r="4"/><path d="M6 21c1-3 3-4 6-4s5 1 6 4"/>',
  ...
  const sfx = (function () {
    ...
    const SOUNDS = {
      ...
      switch: () => { tone(440, 660, 0.08, 'square', 0.12); tone(660, 880, 0.08, 'square', 0.12, 0.07); },
      ...
      click: () => tone(700, 700, 0.03, 'square', 0.08),
    ...
    function setVolume(v) { settings.volume = U.clamp(v, 0, 1); if (master) master.gain.value = settings.muted ? 0 : settings.volume * 0.5; store.saveSettings(settings); }
    function toggleMute() { settings.muted = !settings.muted; if (master) master.gain.value = settings.muted ? 0 : settings.volume * 0.5; store.saveSettings(settings); return settings.muted; }
    return { unlock, play, setVolume, toggleMute, settings };
  })();
